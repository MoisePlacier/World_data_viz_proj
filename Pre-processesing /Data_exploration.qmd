...
```{r}
selectInput("score_dim", "Dimension :", 
            choices = names(vars_groupes_scores),
            selected = "score_global")
```


```{r}
geo_sf <- merge(geo_sf, scores, by = "REF_AREA", all.x = TRUE)
```

```{r}
geo_sf$qualite_soins
```


```{r}
pal <- colorNumeric("RdYlGn", domain = geo_sf$qualite_soins, na.color = "grey90")
...
leafletProxy("map") %>%
  ...
  addPolygons(
    data = geo_sf,
    fillColor = ~pal,
    label = ~paste0(sovereignt, ": ", round(qualite_soins, 2))
  )
```
