...
dt_summary <- dt[, 
  .(median = median(OBS_VALUE,na.rm = TRUE), Tableau = first(Tableau)), 
...
setnames(dt_wide, gsub(" ", "_", names(dt_wide)))
...
na_count <- sapply(dt_wide, function(y) sum(is.na(y)))
...
na_count_dt[, total_count := nrow(dt_wide)]
...
```{r}
valid_indicators <- na_count_dt[prop_na < 0.15, as.character(variable)]
valid_indicators
```
...
```{r}
Mortalite_evitable <- dt[Tableau == "Mortalite evitable"]
Mortalite_evitable
```


```{r}
table(Mortalite_evitable$Measure)
```


```{r}
table(Mortalite_evitable$Unit.of.measure)
```
